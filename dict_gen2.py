...
twitter_dir = os.lidtdir(path) # creates list object of all files in path
...
def main():
    comt_time = init_time() # gets initial time of first tweet

    file_name_inc = 0
    output_file = open("dicts/time" + file_name_inc + ".txt", w) # file to save first time seg
    print("openned: ", output_file.name) # error checking
    ...
    for i in range(0, len(ind)):   # iterates through all files
        ...
        with open(file_path) as f:
            ...
            for j in range(0, len(data)):   # iterates through all data in a file
                ...
                if compt_time.date() < file_time(i, j).date():
                    ...
                    comp_time = f_time(i,j) + timedelta(hours=1)
                else:
                    f.write(data[j]['tweet_text']) # writes the tweet to file
