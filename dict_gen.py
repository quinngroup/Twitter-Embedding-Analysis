        ...
        for i in range(0, len(ind)):   # iterates through ind so files will be check in correct order
            ...
            with open(pt) as d:        # open the f#ilpaces after keyword
                ...
                for j in range(0, len(acc_d)):    # allows us to go  through all the elements in the json
                    if comp_time.date() > f_time(i,j).date():    # checks to see if new time file needs to be created
                        # make a newtext file and write to that one
                        name = "Time" + str(f_name_inc) + ".txt"   # creates new name for file out of its time combination of a date and a time.
                        f_name_inc+=1
                        print("time limit:", comp_time, "curent file time:", f_time(i,j))
                        f.close()
                        print("opened:", name)
                        f = open("dicts/" + name, "w")         # sets f to be the new file
                        f.write(acc_d[j]['tweet_text']) # writes the tweets to the text
                        comp_time =  f_time(i,j) + timedelta(hours=1)    #   updates time if time is past 12 hours
                    else:
                        f.write(acc_d[j]['tweet_text']) # writes the tweets to the text
